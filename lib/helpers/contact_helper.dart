...
class ContactHelper {
  ...
  factory ContactHelper() =>
      _instance; // obtém objeto de qualquer parte do code
  ...
  Future<Database> initDb() async {
    ...
    final path = join(databasesPath, "contacts.db"); // caminho bd

    return await openDatabase(path, version: 1,
        onCreate: (Database db, int newerVersion) async {
      await db.execute(// ABRINDO BD na primeira vez
  ...
  Future<int?> getNumber() async {
    // quantidade de elementos da table
    Database? dbContact = await db;
